...
def pitcher_lookup(pitcher):
    try:
        ...
        # Pull data from 2025
        data = statcast_pitcher(start_dt='2025-03-01', end_dt='2025-08-07', player_id=pitcher.player_id)
...
def main():
    pitcher = Pitcher('Jackson', 'Jobe')
    batter = Batter('Jose', 'Ramirez')
    pitcher = pitcher_lookup(pitcher)
    batter = batter_lookup(batter)
    pitcher.show_total_count_usage()
